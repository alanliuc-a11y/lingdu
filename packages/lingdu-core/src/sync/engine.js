...
class SyncEngine extends EventEmitter {
  constructor(config) {
    ...
    this.profilesDir = config.profilesDir || path.join(require('os').homedir(), '.openclaw', 'workspace');
    this.watcher = null;
    this.ws = null;
  ...
  async handleScene(scene, serverContent) {
    switch (scene) {
      ...
      case SCENE_NO_DATA:
        ...
        return { success: false, message: '未检测到灵魂数据，请初始化配置后重试' };
  ...
  startWatching() {
    ...
    console.log(`[LingDu] Watching: ${this.profilesDir}`);
  }
  ...
  stopWatching() {
    if (this.watcher) {
      this.watcher.close();
      this.watcher = null;
      console.log('[LingDu] File watching stopped');
    }
  }

  async connectWebSocket() {
    const wsUrl = this.config.cloud_url.replace(/^https?/, 'wss') + '/ws';
    console.log(`[LingDu] Connecting WebSocket: ${wsUrl}`);
    ...
    try {
      ...
      this.ws.on('close', () => {
        ...
        setTimeout(() => {
          if (!this.connected) {
            console.log('[LingDu] Reconnecting WebSocket...');
      ...
      this.ws.on('error', (err) => {
        console.error('[LingDu] WebSocket error:', err.message);
      });
    } catch (e) {
      console.error('[LingDu] WebSocket connection failed:', e.message);
    }
  }
